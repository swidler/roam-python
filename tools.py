...
class Tee:
    """Write stdout to multiple streams at once."""
...
def start_stdout_tee(path, tee_stderr=False):
    """If path is given, keep stdout on screen and also write it to path.

    Returns
    -------
    handle : file handle or None
    orig_stdout : original sys.stdout
    orig_stderr : original sys.stderr
    """
    orig_stdout = sys.stdout
    orig_stderr = sys.stderr

    if not path:
        return None, orig_stdout, orig_stderr
    ...
    sys.stdout = Tee(orig_stdout, handle)

    if tee_stderr:
        sys.stderr = Tee(orig_stderr, handle)

    return handle, orig_stdout, orig_stderr


def stop_stdout_tee(handle, orig_stdout, orig_stderr):
    """Restore stdout/stderr and close the tee file if needed."""
...
def nanmerge(arr, operation):
    ...
    if operation == "average":
        ...
        for i in range(0, len(arr), 2):
            take_me = np.nanmax(
                np.array([arr[i], arr[i + 1]])
            )  # regular max function will always take nan
    ...
    for i in range(0, len(arr), 2):
        if np.isnan(arr[i]):
            ...
            if np.isnan(arr[i + 1]):
                ...
                )  # num + num = (num + num) * factor. yields float result, with
                result.append(
                    merged
                )  # sometimes strange precision (eg 0.8200000000000001)
    return result
...
def standardize_region(region):
    ...
    std_region = {"chrom": chrom, "start": start, "end": end}
    return std_region
...
def do_conv(vec, nans, tpl, mode, zero_for_nan):
    ...
    vec[nans] = 0
    res = (
        np.convolve(vec, tpl, mode) / zero_for_nan
    )  # raises divide by zero runtime warnings
    return res
...
def bmm(H, p, w, tolerance, p_known):
    ...
    while (llike[-2] - llike[-1]) / llike[-2] > tolerance:
        # update rgk
        nom = (
            (np.array([w]).T) * np.ones(N + 1)
        ) * B  # this will work only if dimensions match
        # is above numerator?
...
def nan_eq_2d(list1, list2):
    for x in range(len(list(list1))):
        for y in range(len(list(list1[x]))):
            if list1[x][y] != list2[x][y] and (
                ~np.isnan(list1[x][y]) or ~np.isnan(list2[x][y])
            ):
                print(
                    f"list1({x},{y}) is {list1[x][y]} and list2({x},{y}) is {list2[x][y]}"
                )
                return
...
def ancient_Newton_Raphson(max_iterations, min_tol, pi, tij, nij):
    ...
    while iteration < max_iterations and tol > min_tol:
        ...
        with np.errstate(divide="ignore", invalid="ignore"):
            term1 = (
                A * pi_col / den
            )  # contribution to dℓ/dm, shape (no_samples, no_pos)
    ...
    for chrom in chroms:
        ...
        for samp in range(no_samples):
            ...
            [tij[samp], nij[samp]] = samples[samp].smooth(
                chr_idx[samp], int(win_size[samp])
            )
...
def _cmap(color_blind=False, mode="20and80are0") -> ListedColormap:
    ...
    if mode == "20and80are0":
        r = np.concatenate(
            [
                np.full(20, r_ext[0]),
                np.linspace(r_ext[0], r_ext[1], 31),
                np.full(50, r_ext[1]),
            ]
        )
        g = np.concatenate(
            [
                np.full(50, g_ext[0]),
                np.linspace(g_ext[0], g_ext[1], 31),
                np.full(20, g_ext[1]),
            ]
        )
        b = np.linspace(b_ext[0], b_ext[1], 101)
    else:
        r = np.linspace(r_ext[0], r_ext[1], 101)
        g = np.linspace(g_ext[0], g_ext[1], 101)
        b = np.linspace(b_ext[0], b_ext[1], 101)
    return ListedColormap(np.stack([r, g, b], axis=1), name="methyl_20_80")


def _first_geq_idx(vec, bp):
    return int(np.searchsorted(vec, bp, side="left"))


def _last_leq_idx(vec, bp):
    return int(np.searchsorted(vec, bp, side="right") - 1)


def _region_bed(chrom, start, end):
    return pbt.BedTool(f"{chrom}\t{start}\t{end}\n", from_string=True)
        ...
        if "positions" in groups:
            ...
            for n in nums:
                if n not in uniq:
                    uniq.append(n)
            name_map = {
                n: (names[i] if i < len(names) else f"group{n}")
    ...
    *,
    ...
    def draw_interval_track(ax, bedtool, name, directional, bar_frac=1.0):
        ...
        def arrow(side, y0, h):
            ...
            ys = (np.array([0, 0, 0.5, 1, 1, 0.5]) * h) + y0
        ...
        def _bp_to_ix(bp):
            ...
            return float(
                np.clip(
                    np.interp(float(bp), ys, xs, left=xs[0], right=xs[-1]),
                    xs[0],
                    xs[-1],
                )
            )
        ...
        if directional:
            ...
            feats = [
                SimpleNamespace(start=a, end=b, name=nm, strand=s)
            ...
            def _draw_strand_lanes(lanes, base_y):
                ...
                for li, lane in enumerate(lanes):
                    ...
                    for feat in lane:
                        ...
                        ov_a = max(gbp, start_bp)
                        ov_b = min(ebp, end_bp)
                        has_cols = bool(
                            np.any((gc_chr_coords >= ov_a) & (gc_chr_coords <= ov_b))
                        )
                        label = feat.name
                        if has_cols:
                            ...
                            if label:
                                ok_in_bar = (w >= min_w_for_label) and (
                                    ibc > last_right + min_gap
                                )
                                if ok_in_bar:
                                    txt = ax.text(
                                        ibc + w / 2,
                                    ...
                                    if (
                                        y_text > top_of_block - 0.02
                                    ):  # too close to top of this strand block → put below
                                        y_text = y0 - pad_y

                                    # center horizontally on the bar, clamped to the panel
                                    x_text = float(
                                        np.clip(ibc + w / 2, beg_i + 2, fin_i - 2)
                                    )
                            ...
                            txt = ax.text(
                                x_center,
                                y0 + 0.5 * lane_h,  # center on the lane
                                label,
                                ha="center",
                                va="center",
                                color="k",
            ...
            for f in merged:
                ...
                if ibc <= iec:
                    ...
                    if gbp < start_bp:
                        arrow("L", y0, float(bar_frac))
                    if ebp > end_bp:
                        arrow("R", y0, float(bar_frac))
    ...
    axx.set_xticklabels(
        [_comma(int(gc_chr_coords[beg_i])), _comma(int(gc_chr_coords[fin_i]))],
        fontsize=9,
    )
