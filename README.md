...
IMPORTANT: If the sample is aligned to any genome version but Hg19, it is important to
	...
	-ct, --max_c_to_t user-defined threshold to identify sites with a true C->T mutation (used only
 		if no_diag_filt or thresh_upper is specified). Ignored for any coverage in which it conflicts with min_t 
 		(default: 0.25).
...
for DMRs between them. Each group can contain ancient samples.
...
Apart from these three mandatory paramters, a modern reference methylation map should be
	...
	are not aligned to hg19 a new file must be provided), RoAM will use the	reference defined 
 	in the config file of the DNA methylation reconstruction pipeline, whose path can be 
  	specified with the parameter -rco. If no such config file exists, simulations
