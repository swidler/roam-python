...
argParser.add_argument(
    "-g", "--groups", nargs="+", help="group names--should correspond with samples"
)
argParser.add_argument(
    "-o",
    "--object_dir",
...
argParser.add_argument(
    "-du", "--dump_dir", help="directory for output txt files and pics"
)
...
argParser.add_argument(
    ...
    "--prom_def",
    nargs="+",
    help="promoter definition around TSS: a list of 2 values [before, after], where before is the number of nucleotides into the intergenic region, and after is the number of nucleotides",
)
argParser.add_argument(
    "-di",
    "--dmr_infile",
    help="pickled DMR file for use in fdr, permutations, and plots",
)
argParser.add_argument(
    "-dpi", "--dmp_infile", help="pickled DMR permutation file for use in permutstat"
)
argParser.add_argument(
    "-t", "--templ", help="template to match any extra text in sample filename"
)
...
argParser.add_argument(
    ...
    "--delta",
    type=float,
    help="minimum methylation difference between the two groups",
)
argParser.add_argument(
    "-mc",
    "--min_cpgs",
    type=int,
    help="DMRs whose number of CpGs is less than min_CpGs are filtered out",
)
argParser.add_argument(
    "-mq", "--min_qt", type=int, help="DMRs with Qt < min_qt are filtered out"
)
argParser.add_argument(
    "-mb", "--min_bases", type=int, help="DMRs shorter than min_bases are filtered out"
)
argParser.add_argument(
    "-mad",
    "--max_adj_dist",
    type=int,
    help="max distance between adjacent CpGs within the same DMR",
)
argParser.add_argument(
    "-mf",
    "--min_finite",
...
argParser.add_argument(
    "-p", "--permutations", type=int, help="number of permutations to run"
)
argParser.add_argument(
    "-dmi", "--dmr_idx", type=int, help="index of DMR"
)  # index or name??
...
argParser.add_argument(
    "-r",
    "--noref",
    help="flag for using reference genome for histogram matching",
    action="store_true",
)
argParser.add_argument(
    "-re", "--noreport", help="flag for logging info", action="store_true"
)
argParser.add_argument(
    "-an", "--noannot", help="flag for running annotation", action="store_true"
)
argParser.add_argument(
    "-pr",
    "--propinf",
    type=float,
    help="Minimum fraction of informative samples per group in DMR",
)
argParser.add_argument(
    "-sc",
    "--min_dmrcov",
    type=int,
    help="Minimum mean coverage per CpG at DMR per sample",
)
argParser.add_argument(
    "-wi",
    "--widenby",
    type=int,
    help="Flanking region size in bp for individual DMR plot",
)
argParser.add_argument(
    "-so",
    "--stdout_file",
    help="also save stdout/stderr to this file while keeping them on screen",
)
...
try:
    samples = (
        ...
        else config["basic"]["samples"].split(",")
    )
    mod_samples = (
        parameters["mod_samples"[:]]
        if "mod_samples" in parameters
        else config["basic"]["mod_samples"].split(",")
    )
    chroms = (
        parameters["chroms"[:]]
        if "chroms" in parameters
        else config["basic"]["chromosomes"].split(",")
    )
    templ = parameters["templ"] if "templ" in parameters else config["files"]["templ"]
    object_dir = (
        parameters["object_dir"]
        if "object_dir" in parameters
        else config["paths"]["object_dir"]
    )
    stages = (
        parameters["stages"[:]]
        if "stages" in parameters
        else config["basic"]["stages"].split(",")
    )
    gc_object = (
        parameters["gc_file"]
    ...
    min_fin = (
        parameters["min_finite"]
        if "min_finite" in parameters
        else list(map(int, config["basic"]["min_fin"].split(",")))
    )
    win_size = (
        parameters["win_size"]
    ...
    group_names = (
        ...
        else config["basic"]["group_names"].split(",")
    )
    min_CpGs = (
        parameters["min_cpgs"]
        if "min_cpgs" in parameters
        else config["basic"].getint("min_CpGs")
    )
    min_Qt = (
        parameters["min_qt"]
        if "min_qt" in parameters
        else config["basic"].getint("min_Qt")
    )
    min_bases = (
        parameters["min_bases"]
        if "min_bases" in parameters
        else config["basic"].getint("min_bases")
    )
    max_adj_dist = (
        parameters["max_adj_dist"]
        if "max_adj_dist" in parameters
        else config["basic"].getint("max_adj_dist")
    )
    delta = (
        parameters["delta"]
        if "delta" in parameters
        else float(config["basic"]["delta"])
    )
    bismark_infile = (
        parameters["bismark"]
        if "bismark" in parameters
        else rconfig["files"]["bismark_infile"]
    )
    modern = (
        parameters["modern"]
        if "modern" in parameters
        else rconfig["files"]["modern_infile"]
    )
    mod_name = (
        parameters["mname"] if "mname" in parameters else rconfig["modern"]["mod_name"]
    )
    mod_species = (
        parameters["mspecies"]
        if "mspecies" in parameters
        else rconfig["modern"]["mod_spec"]
    )
    mod_ref = (
        parameters["mref"] if "mref" in parameters else rconfig["modern"]["mod_ref"]
    )
    mod_method = (
        parameters["mmethod"]
        if "mmethod" in parameters
        else rconfig["modern"]["mod_method"]
    )
    gene_file = (
        parameters["gene_file"]
        if "gene_file" in parameters
        else config["files"]["gene_file"]
    )
    cgi_file = (
        parameters["cgi_file"]
        if "cgi_file" in parameters
        else config["files"]["cgi_file"]
    )
    cust_file1 = (
        parameters["cust_file1"]
        if "cust_file1" in parameters
        else config["files"]["cust_file1"]
    )
    cust_file2 = (
        parameters["cust_file2"]
        if "cust_file2" in parameters
        else config["files"]["cust_file2"]
    )
    prom_def = (
        parameters["prom_def"[:]]
        if "prom_def" in parameters
        else config["basic"]["prom_def"].split(",")
    )
    dump_dir = (
        parameters["dump_dir"]
        if "dump_dir" in parameters
        else config["paths"]["dump_dir"]
    )
    log_dir = (
        parameters["log_dir"] if "log_dir" in parameters else config["paths"]["log_dir"]
    )
    ...
    por = (
        parameters["propinf"]
        if "propinf" in parameters
        else float(config["basic"]["por"])
    )
    mcpc = (
        parameters["min_dmrcov"]
        if "min_dmrcov" in parameters
        else config["basic"].getint("mcpc")
    )
    ...
    if "DMR" in stages or "permute" in stages or "plot" in stages:
        ...
        chr_names = (
            chroms if chroms and chroms[0] != "" else gc.chr_names
        )  # assumes user wants all chroms (or all but x)
    ...
    if "DMR" in stages:
        ...
        "fdr" in stages
        or "permute" in stages
        or "permutstat" in stages
        or "plotmethylation" in stages
        or "plot" in stages
    ):
        DMR_obj_infile = (
            parameters["dmr_infile"]
            if "dmr_infile" in parameters
            else config["files"]["DMR_obj_infile"]
        )
    ...
    if "DMR" in stages or "fdr" in stages:
        ...
        if bismark_infile:
            mms.create_mms_from_bismark_file(
                bismark_infile, gc_object, mod_name, mod_species, mod_ref, mod_method
            )
        else:
        ...
        if report:
            if annot:
                dms.annotate(
                    gene_file,
                    cgi_file,
                    cust_bed1=cust_file1,
                    cust_bed2=cust_file2,
                    prom_def=prom_def,
                )
    ...
    if "fdr" in stages:
        ...
        win_size_orig = alg_props[
            "win_size"
        ]  # save values from original groupDMRs process
        ...
        chr_names = [x for x in chr_names if "X" not in x]  # remove chrx from list
        sim_permutations = (
            parameters["sim_permutations"]
            if "sim_permutations" in parameters
            else config["permute"].getint("sim_permutations")
        )
        thresh = (
            parameters["thresh"]
            if "thresh" in parameters
            else float(config["basic"]["thresh"])
        )
        ...
        for samp in samplist:  # Ancient samples
            ...
            print(
                "Average methylation in ",
                samp.name,
                "ref:",
                np.nanmean(np.concatenate(sref.methylation).ravel()),
            )
            refd[samp.name] = sref
            del sref
        mod_refd = {}
        ...
        for perm in range(sim_permutations):
            ...
            for sample in samplist:
                ...
                samp_copy.reconstruct_methylation(
                    ref=mms, function=method, win_size=win_size, lcf=lcf, **m_params
                )
            ...
            dmr_obj_list[perm].groupDMRs(
                win_size=win_size_orig,
                lcf=lcf_orig,
                samples=samps,
                sample_groups=group_names,
                coord=gc,
        ...
        adjusted_DMR = dms.adjust_params(
            dmr_obj_list, thresh=thresh, fname=logfile, statfile=statfile
        )
        if annot:
            adjusted_DMR.annotate(
                gene_file,
                cgi_file,
                cust_bed1=cust_file1,
                cust_bed2=cust_file2,
    ...
    if "permute" in stages:
        num_permutations = (
            ...
            else config["permute"].getint("num_permutations")
        )
    ...
    if "permutstat" in stages:
        if "permute" not in stages:
            DMP_obj_infile = (
                parameters["dmp_infile"]
                if "dmp_infile" in parameters
                else config["files"]["DMP_obj_infile"]
            )
    ...
    DMR_chrom = (
        parameters["dmr_chrom"]
        if "dmr_chrom" in parameters
        else config["plotmethylation"]["DMR_chrom"]
    )
    DMR_idx = (
        parameters["dmr_idx"]
        if "dmr_idx" in parameters
        else config["plotmethylation"].getint("DMR_idx")
    )
    ...
    if "plot" in stages:  # deal with custom files here?
        widenby = (
            parameters["widenby"]
            if "widenby" in parameters
            else config["plotmethylation"].getint("widenby")
        )
